...
<!-- EL표기법: jsp에서 사용되는 표현식을 더 간결하게 사용할 수 있도롥 해주는 표현 형태 
	 EL(Expression Language) - EL로 값을 표현하려면 jsp의 관리하에 있는 4개의 영역에서만 사용을 할 수 있다
	 1) page영역: 단 하나의 페이지(현재 페이지)에서만 값을 저장하고 공유 할 수 있다.
	 2)	request영역(가장 많이 쓰임) : 지역 개념으로 페이지가 종료되면 영역도 종료가 된다.
	 						         최대 2개의 페이지에서 값을 저장하거나 공유할 수 있다.
	 3) session영역 : 전역 개념으로 브라우저가 완전히 종료되기 전까지는 영역이 살아 있다.
